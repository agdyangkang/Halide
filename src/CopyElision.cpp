...
string print_function(const Function &f) {
    ...
    if (f.has_extern_definition()) {
        // TODO(psuriana): Handle print for extern function
...
string get_pointwise_copy_producer(const Function &f,
    ...
    if (f.has_extern_definition() &&
        ...
        debug(0) << "\t\tFOUND halide_buffer_copy: ";
...
vector<CopyPair> get_pointwise_copies(const map<string, Function> &env) {
    ...
    // TODO(psuriana): We should only consider the case when the function
    // only has 1 caller
